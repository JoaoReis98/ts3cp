        ...
        @forelse($virtual_servers as $virtual_server)
            ...
            <div class="col-md-4 col-sm-6 col-xs-12">
                <div class="info-box bg-{{ ($virtual_server->server->success != false && $virtual_server->success != false) ? 'green' : 'red' }}">
                    <a class="remove-link-color" href="{{ route('virtualserver_view', $virtual_server->id) }}"><span class="info-box-icon"><i class="fa fa-expand"></i></span></a>

                    <div class="info-box-content">
                        <span class="info-box-text">{{ $virtual_server->serverInfo['virtualserver_name'] }}</span>
                        <span class="info-box-number">{{ $virtual_server->server->ip }}:{{ $virtual_server->port }}
                            <span class="pull-right"> <?php if($virtual_server->success) { echo $virtual_server->getSlots(); } else { echo "0"; } ?> / {{ $virtual_server->serverInfo['virtualserver_maxclients'] }}</span> </span>
                        <div class="progress">
